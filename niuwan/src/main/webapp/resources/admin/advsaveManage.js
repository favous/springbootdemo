...
function beforeNow(){
	...
	$.each($(".badge"), function(i, item){      
		console.log(parseFloat(item.id)+"==="+parseFloat(nowDate));
...
Date.prototype.Format = function (fmt) { // author: meizz
  	...
  	if (_fileName !== "png" && _fileName !== "jpg") {
  		...
  		$("#addAdv").ajaxSubmit({
  			...
  			success : function(data) {
  				if(data.success=="success"){
  					if(data.type==1){
  						$("#a_kp1Img").html("<img src='"+data.outImgPreFixUrl+data.picPath+"'  width='140px' height='246px' style='top: 43px;left: 26px;position: absolute'>");
  						...
  						$("#a_cpImg").html("<img src='"+data.outImgPreFixUrl+data.picPath+"' width='140px' height='48px'style='top: 205px;left: 26px;position: absolute'>");
  ...
  function addAdv() {
	...
	var totaldisNum = $.trim($("#totaldisNum").val());
	 var radio_tag =document.getElementsByName("type");
