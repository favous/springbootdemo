	...
	//提示信息
		...
		var table = $("#dataTable").dataTable({
			...
			ajax : function(data, callback, settings) {
				...
				//查询条件
				param.articleTitle = $.trim($("#searchName").val());
				param.status = $.trim($("#searchState").val());
				//console.log(param);
				//ajax请求数据
				$.ajax({
					type : "GET",
			...
			columns : [
				{
					...
					data : null,
					render : function(data, type, row, meta) {
						var tdStr = '<img src="'+data.articleCoverPic+'" width="60px" height="30px">';
						return tdStr;
					}
				}, {	
					sTitle : '标题',
					...
					render : function(data, type, row, meta) {
						var tdStr = "<div class='btn-group'>";
							tdStr += "<button type=\"button\" class=\"btn btn-default\" onclick=\"dtl('" + data.id + "')\">详请</button>";
							tdStr += "<button type=\"button\" class=\"btn btn-default\" onclick=\"toModify('" + data.id + "')\">编辑</button>";
