...
<body class="flat-blue">
							...
							<div class="card">
								<div class="card-header">
									<div class="card-title">
										<div class="row">
											<form role="form" class="form-horizontal" id="searchForm" action="<%=basePath%>article/articleManage" method="post">
												<div class="col-xs-3">
													<div class="input-group" style="margin-top: 5px;">
														<span class="input-group-addon">文章标题</span> <input type="text" class="form-control" name="name" id="searchName" value="${name}" autocomplete="off">
													</div>
												</div>
												<div class="col-xs-3">
													<div class="input-group" style="margin-top: 5px;">
														<span class="input-group-addon">状态</span> <select class="form-control" id="searchState" name="state">
															<option value="0">全部</option>
															<option <c:if test="${state=='10' }">selected="selected"</c:if> value="10">待审核</option>
															<option <c:if test="${state=='20' }">selected="selected"</c:if> value="20">审核驳回</option>
															<option <c:if test="${state=='30' }">selected="selected"</c:if> value="30">审核通过</option>
														</select>
													</div>
												</div>
												<div class="col-xs-1">
	...
	<script type="text/javascript" src="<%=basePath%>resources/admin/articleManage.js"></script>
	<script type="text/javascript">
	//显示详细
	function dtl(id){
		window.location.href='<%=basePath%>article/articleDtl?id='+id;
	}
	function del(id){
		var se=confirm("请确认是否删除!");
		if (se==true)
		  {
			window.location.href='<%=basePath%>article/articleDel?id='+id;
		  }
		else{}
	}
	function toModify(id){
		window.location.href='<%=basePath%>article/articleModify?id='+id;
	}
	</script>
</body>
</html>
