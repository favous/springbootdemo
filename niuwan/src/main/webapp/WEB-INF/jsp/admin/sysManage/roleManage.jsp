...
<body class="flat-blue">
							...
							<div class="card">
								<div class="card-header">
									<div class="card-title">
										<form role="form" class="form-horizontal" id="searchForm" action="<%=basePath%>admin/roleManage" method="post">
											<div class="row">
												<div class="col-xs-4">
												...
												<div class="col-xs-1">
													<button type="button" class="btn btn-info btn-block" onclick="toModify()">新增</button>
												</div>
											</div>
										</form>
									</div>
										...
										<tbody>
											<c:forEach items="${roles}" var="role" varStatus="status">
												<tr>
													...
													<td style="width: 30%">
														<div class="btn-group">
															<button type="button" class="btn btn-default" <c:if test="${role.id<=1}">disabled="disabled"</c:if> onclick="toModify('${role.id}')">修改</button>
															<button type="button" class="btn btn-default" <c:if test="${role.id<=1}">disabled="disabled"</c:if> onclick="toDelete('${role.id}')">删除</button>
														</div>
													</td>
												</tr>
											</c:forEach>
	...
	<div class="modal fade" id="modifyModal" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					<h4 class="modal-title" id="modifyAdd">修改角色信息</h4>
				</div>
				<div class="modal-body">
					<form id="modifyRole">
						<input type="hidden" id="mid" name="mid"> <input type="hidden" id="rid" name="id">
						<div class="form-group">
							<label for="name">角色</label> <input type="text" class="form-control" name="role_name" id="name" placeholder="请输入账号" onkeyup="keyUp(this)">
						</div>
						<div class="form-group">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-info" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-danger" onclick="modifyRole()">提交</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<script type="text/javascript" src="<%=basePath%>resources/admin/roleManage.js"></script>
</body>
</html>
