...
<body class="flat-blue">
	<div class="app-container">
		<div class="row content-container">
			...
			<div class="container-fluid">
				<div class="side-body">
					<div class="page-title">
						<span class="title">修改广告计划</span>
					</div>
					<div class="col-xs-12">
						<div class="card">
							<form class="form-horizontal" role="form" id="modifyAdvForm">
								<input type="hidden" id="imgType" name="imgType" value="">
								<input type="hidden" id="id" name="id"  value="${advPlan.id}"> 
								<input type="hidden" id="typesss" name="typesss" value="${advPlan.type}"> <input type="hidden" id="timeList" name="timeList" value=""><input type="hidden" id="hiddenUrl" name="hiddenUrl" value="<%=basePath%>">
								<div style="height: 20px"></div>
								<div class="form-group">
									<label for="firstname" class="col-sm-1 control-label">计划名称</label>
									<div class="col-sm-10">
										<input type="text" name="name" id="name" class="form-control" value="${advPlan.name}" placeholder="请填写计划名称..">
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-10">
										<div class="col-sm-5" style="padding: 0px">
											<input type="date" name="startTime" id="startTime" class="form-control" value="${advPlan.startdate}" placeholder="请选择开始日期.." onchange="timeOK()">
										</div>
										<div class="col-sm-5">
											<input type="date" name="endTime" id="endTime" class="form-control" value="${advPlan.enddate}" placeholder="请选择结束日期.." onchange="timeOK()">
										</div>
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="expectedNum" id="expectedNum" class="form-control" value="${advPlan.expectedNum}" placeholder="请填写期望展示数..">
									</div>
								</div>
								<div class="form-group">
									<label for="firstname" class="col-sm-1 control-label">总展示数</label>
									<div class="col-sm-10">
										<input type="text" name="totaldisNum" id="totaldisNum" class="form-control" value="${advPlan.totaldisNum}" placeholder="请填写总展示数...">
									</div>
								</div>
								<div class="form-group">
									<label for="firstname" class="col-sm-1 control-label">总点击数</label>
									<div class="col-sm-10">
										<input type="text" name="totalNum" id="totalNum" class="form-control" value="${advPlan.totalNum}" placeholder="请填写总点击数...">
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-5">
										...
										<div class="col-sm-7" style="padding: 0px">
											<input type="text" name="dispalyNum" id="dispalyNum" class="form-control" value="${advPlan.dispalyNum}" placeholder="1~100次，0代表不限">
										</div>
									</div>
									<div class="col-sm-5">
										<span class="col-sm-3" style="padding-top: 7px; padding-left: 0px;">同一用户，点击次数</span>
										<div class="col-sm-7" style="padding: 0px">
											<input type="text" name="clickNum" id="clickNum" class="form-control" value="${advPlan.clickNum}" placeholder="1~100次，0代表不限">
										</div>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-1">
										<span><input name="type" type="radio" value="1" <c:if test="${advPlan.type==1 }">checked="checked"</c:if> />开屏页 </span>
									</div>
									<div class="col-sm-1">
										<span><input name="type" type="radio" value="2" <c:if test="${advPlan.type==2 }">checked="checked"</c:if> />banner </span>
									</div>
									<div class="col-sm-1">
										<span><input name="type" type="radio" value="3" <c:if test="${advPlan.type==3 }">checked="checked"</c:if> />插屏 </span>
									</div>
								...
								<div class="form-group">
								<input type="hidden" id="nowtype" value="${advPlan.type }">
									<label for="firstname" class="col-sm-1 control-label">页面预览</label>

									<div class="col-sm-10">
										<div id="bannerImg" class="col-sm-5" <c:if test="${advPlan.type!='2' }">style="display: none"</c:if>>
											<div  class="col-sm-1">
											<span>banner：</span> 
											</div>
											<div class="col-sm-5">
											...
											<div id="a_bannerImg">
												<c:if test="${advPlan.type==2}">
													<img src="${outImgPreFixUrl }${advPlanImg[0].url}" width='83px' height='49px' style='top: 71px;position: absolute; left:55px'>
												</c:if>
											</div>
										</div>
									</div></div>
									<div class="col-sm-10">
										<div id="kp1Img" class="col-sm-5" <c:if test="${advPlan.type!='1' }">style="display: none"</c:if>>
											...
											<div id="a_kp1Img">
												<c:if test="${advPlan.type==1}">
													<img src="${outImgPreFixUrl }${advPlanImg1}" width='140px' height='246px' style='top: 42px; position: absolute;left: 26px'>
												</c:if>
											</div>
										</div></div>
										<div id="kp2Img" class="col-sm-5" <c:if test="${advPlan.type!='1' }">style="display: none"</c:if>>
										...
										<input type="hidden" id="b_kp2Img" name="b_kp2Img">
											<div id="a_kp2Img">
												<c:if test="${advPlan.type==1}">
													<img src="${outImgPreFixUrl }${advPlanImg2}" width='140px' height='246px' style='top: 42px; position: absolute; left: 26px'>
												</c:if>
											</div>
										</div>
									</div>
									...
									<div class="col-sm-10">
										<div id="cpImg" class="col-sm-5" <c:if test="${advPlan.type!='3' }">style="display: none"</c:if>>
											...
											<div id="a_cpImg">
												<c:if test="${advPlan.type==3}">
													<img src="${outImgPreFixUrl }${advPlanImg[0].url}" width='139px' height='48px' style='top: 205px;position: absolute; left: 68px'>
												</c:if>
											</div>
										</div>
									</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="link" id="link" class="form-control" value="${advPlan.link}" placeholder="请填写落地链接..">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="introduction" id="introduction" class="form-control" value="${advPlan.introduction}" placeholder="请填写简单介绍..">

									</div>
								</div>
								<div class="form-group">
									<div class="col-sm-offset-2 col-sm-10">
										<button type="button" onclick="modifyAdv()" class="btn btn-default">确认修改</button>
									</div>
								</div>
							</form>
						</div>
			...
			<script type="text/javascript" src="<%=basePath%>resources/admin/advModify.js"></script>
</body>
</html>
